...
void ScanningSystem::update(float dt) {
	...
	for (auto e=registered_entities.begin(); e != registered_entities.end(); e++) {
		...
		for (int i=0; i<num_scannable; i++) {
			...
			//std::cout << "Scannable object with type " << s.scan_value << std::endl;
			...
			if (s.scan_value == scanning_for.sought_scan_value) {
				...
				std::cout << ent << " found scanned entity " << scanned_ent << " with pos " << p.x << ", " << p.y << std::endl;

				//component_manager->get_component_array<Persuing>()->add_entity(ent, { p.x, p.y });

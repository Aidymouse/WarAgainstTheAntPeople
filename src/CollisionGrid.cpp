...
void CollisionGrid::insert_entity( std::shared_ptr<Entity> ent) {
	...
	std::string cell_id = std::to_string(cell_row) + ":" + std::to_string(cell_col);
...
std::vector<grid_cell*> CollisionGrid::get_cells_within(float distance, float x, float y) {
	...
	//std::cout << dist_to_top << ", " << dist_to_bottom << ", " << dist_to_left << ", " << dist_to_right << std::endl;
	//std::cout << cells_up << ", " << cells_down << ", " << cells_left << ", " << cells_right << std::endl;

	std::vector<grid_cell*> retrieved_cells;

	for (int row = cell_row - cells_up; row <= (cell_row + cells_down); row++) {
		for (int col = cell_col - cells_left; col <= (cell_col + cells_right); col++) {

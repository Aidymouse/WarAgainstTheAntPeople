...
struct {
  ...
  animation bomb8 = {bomb_graphics_rects[8], MAX_FUSE_TIMER / 8, &explode1, 8,
                     8};
...
void Bomb::update(float dt) {
  ...
  if (timer <= 0 && !exploded) {
    ...
    std::vector<grid_cell *> exploded_cells =
        Gamestate::main_grid.get_cells_within(EXPLOSION_RADIUS, pos.x, pos.y);

    for (auto &cell : exploded_cells) {
      for (auto &entity : *cell) {
        float dist = Helper::dist_tween_points(entity->pos.x, entity->pos.y,
                                               pos.x, pos.y);

...
public:
	void add_entity(Entity id, T data) {
		//std::cout << "Adding entity (" << id << ") data to spot " << num_components << std::endl;
	...
	void remove_entity(Entity id) {
		...
		//std::cout << "Removing entity (" << id << ") data from spot " << old_component_idx << std::endl;

		components[old_component_idx] = components[num_components-1];

		//Entity swap_entity = component_idx_to_entity[num_components-1]; // When an entity is destroyed, the latest entity becomes the one that was just destroyed
	...
	T get_data(Entity id) {
		//std::cout << "Getting Entity (" << id << ") data from spot " << entity_to_component_idx[id] << std::endl;
	...
	void print_carray() {
		...
		for (int i=0; i<num_components; i++) {
			std::cout << "Component " << i << " (entity " << component_idx_to_entity[i] << ")" << std::endl;
		}
	}

	void entity_destroyed(Entity id) override {
		remove_entity(id);
	}
};

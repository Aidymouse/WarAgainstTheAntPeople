...
public:
	...
	template <typename T> std::shared_ptr<ComponentArray<T>> get_component_array() {
		return std::static_pointer_cast<ComponentArray<T>>(component_arrays[type_to_component_array[typeid(T)]]);
	}
	...
	void entity_removed(Entity id) {
		for (int i=0; i<MAX_COMPONENTS; i++) {
			if (component_arrays[i] != NULL) component_arrays[i]->entity_destroyed(id);
		}
	}
